...
import { Camera, Scan, CheckCircle, Clock, Truck, Loader2, QrCode, X, Check, RefreshCw } from "lucide-react"
...
export function CourierTrackingTab({ userRole }: CourierTrackingTabProps) {
  ...
  const handleHandToCourier = async () => {
    ...
    try {
      ...
      const response = await apiClient.patch(`/outgoing/qr/${scannedQR}/status`, {
        status: "DISPATCHED"
      })
      ...
      if (response.data.success) {
        ...
        toast({
          ...
          description: response.data.error || "Failed to hand letter to courier",
          variant: "destructive",
        })
      }
    } catch (error: any) {
      if (error.response?.status === 404) {
        toast({
          ...
          description: "No outgoing letter found with this QR code. Please check the QR code and try again.",
          variant: "destructive",
        })
      } else {
        toast({
          title: "Handover Failed",
          description: error.response?.data?.error || "Failed to hand letter to courier. Please try again.",
          variant: "destructive",
        })
      }
    } finally {
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Scan the QR code on outgoing letters before handing them to courier services</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
                ...
                <div className="mt-3 p-3 bg-white rounded border border-green-100">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm text-gray-900">{scannedLetter.to}</span>
                    </div>
                    <div className="flex items-center justify-between">
          ...
          <div className="p-8 border-2 border-dashed border-gray-300 rounded-lg text-center bg-gradient-to-br from-gray-50 to-gray-100">
            <div className="max-w-sm mx-auto">
              ...
              <p className="text-gray-600 mb-3">Point your camera at the QR code on the outgoing letter</p>
          ...
          {scannedLetter?.status === "DISPATCHED" ? (
            <div className="p-6 bg-gradient-to-r from-blue-50 to-blue-100 border border-blue-200 rounded-lg text-center">
              ...
              <h3 className="text-lg font-semibold text-blue-800 mb-2">Letter Already Dispatched</h3>
              <p className="text-sm text-blue-700 mb-4">
                This letter has already been handed to courier and is in transit.
              </p>
              <Button
                variant="outline"
                size="sm"
            ...
            <div className="p-6 bg-gradient-to-r from-green-50 to-green-100 border border-green-200 rounded-lg text-center">
              ...
              <h3 className="text-lg font-semibold text-green-800 mb-2">Letter Already Delivered</h3>
            ...
            <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
              <div className="text-center mb-4">
                ...
                <h3 className="text-sm font-medium text-green-800 mb-1">Ready to Hand to Courier</h3>
                <p className="text-xs text-green-700">This letter can be handed to courier service</p>
              </div>
              <Button
                ...
                {courierRecords.length === 0 ? (
                  ...
                  courierRecords.map((record) => (
                    <TableRow key={record.id}>
                      <TableCell className="font-mono text-sm">{record.qrCode}</TableCell>
                      <TableCell>{record.department?.name || 'Unknown Department'}</TableCell>
                      <TableCell>{record.to}</TableCell>
                      <TableCell className="max-w-[200px] truncate">{record.subject || 'No subject'}</TableCell>
                  <TableCell>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <TableCell>
                        <div className="text-sm">
                          ...
                          <div className="text-gray-500 text-xs">{new Date(record.createdAt).toLocaleTimeString()}</div>
                        </div>
                      </TableCell>
                      <TableCell>
                        {record.dispatchedDate ? (
                          <div className="text-sm">
                            <div className="font-medium">{new Date(record.dispatchedDate).toLocaleDateString()}</div>
                            <div className="text-gray-500 text-xs">{new Date(record.dispatchedDate).toLocaleTimeString()}</div>
                          </div>
                        ) : (
                  ...
                  <TableCell>
                        {record.deliveredDate ? (
                          <div className="text-sm">
                            <div className="font-medium">{new Date(record.deliveredDate).toLocaleDateString()}</div>
                            <div className="text-gray-500 text-xs">{new Date(record.deliveredDate).toLocaleTimeString()}</div>
                          </div>
                        ) : (
                          <span className="text-gray-400 text-sm">-</span>
                    )}
